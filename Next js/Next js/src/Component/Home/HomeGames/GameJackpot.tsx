...
import { GetJackpotGameListAction } from '@/Redux/Actions/GameAction'
...
const GameJackpot = () => {
    ...
    const GetGameList = useSelector((state: any) => state.GetJackpotGameListState)

    useEffect(() => {
        dispatch(GetJackpotGameListAction({
            "page": 1,
            "pageSize": 20,
            "categories": ["13"]
        }))
    ...
    useEffect(() => {
        ...
        if (gameListData) {
            ...
            setCookie('jackpotGames', JSON.stringify(gameListData), 30);
        } else {
            const jackpotGames: any = getCookie('jackpotGames');
            if (jackpotGames) {
                const data = JSON.parse(jackpotGames);
                setGameData(data);
            }
        }
    }, [GetGameList]);
            ...
            {gameData?.count > 0 ?
                <section className="py-3 py-lg-5 bg-gredient-black hoverTitle">
                    <div className="container-fluid">
                        <h3 className="hTitle hLine">Jackpot Games<a onClick={() => handleNavigate(`${RouteConfig?.GamesList}?title=${encodeID('Jackpot Games')}&category_id=${encodeID(13)}`)} className="viewAll cursor-pointer"><span>See All</span> <i className="icon-arrow-right"></i></a></h3>
