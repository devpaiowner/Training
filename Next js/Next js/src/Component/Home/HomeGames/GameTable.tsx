...
import { GetTableGameListAction } from '@/Redux/Actions/GameAction'
...
const GameTable = () => {
    ...
    const GetGameList = useSelector((state: any) => state.GetTableGameListState)

    useEffect(() => {
        // if (!GetGameList?.data?.data) {
        dispatch(GetTableGameListAction({
            "page": 1,
            "pageSize": 20,
            "categories": ["7"]
        }))
    ...
    useEffect(() => {
        ...
        if (gameListData) {
            ...
            setCookie('tableGames', JSON.stringify(gameListData), 30);
        } else {
            const tableGames: any = getCookie('tableGames');
            if (tableGames) {
                const data = JSON.parse(tableGames);
                setGameData(data);
            }
        }
    }, [GetGameList]);
            ...
            {gameData?.count > 0 ?
                <section className="py-3 py-lg-5 bg-gredient-black hoverTitle">
                    <div className="container-fluid">
                        <h3 className="hTitle hLine">Table Games <a onClick={() => handleNavigate(`${RouteConfig?.GamesList}?title=${encodeID('Table Games')}&category_id=${encodeID(7)}`)} className="viewAll cursor-pointer"><span>See All</span> <i className="icon-arrow-right"></i></a></h3>
