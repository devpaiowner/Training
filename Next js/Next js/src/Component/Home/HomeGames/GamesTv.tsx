...
import { GetJackpotGameListAction, GetTvGameListAction } from '@/Redux/Actions/GameAction'
...
const GameTv = () => {
    ...
    const GetGameList = useSelector((state: any) => state.GetTvGameListState)

    useEffect(() => {
        dispatch(GetTvGameListAction({
            "page": 1,
            "pageSize": 20,
            "merchant_id": ["900", "901"]
        }))
    ...
    useEffect(() => {
        ...
        if (gameListData) {
            ...
            setCookie('tvGames', JSON.stringify(gameListData), 30);
        } else {
            const tvGames: any = getCookie('tvGames');
            if (tvGames) {
                const data = JSON.parse(tvGames);
                setGameData(data);
            }
        }
    }, [GetGameList]);
            ...
            {gameData?.count > 0 ?
                <section className="py-3 py-lg-5 bg-gredient-black hoverTitle">
                    <div className="container-fluid">
                        <h3 className="hTitle hLine">TV Games<a onClick={() => handleNavigate(`${RouteConfig?.Providers}?provider_name=${encodeID('TV Games')}&provider_id=${encodeID([900, 901])}`)} className="viewAll cursor-pointer"><span>See All</span> <i className="icon-arrow-right"></i></a></h3>
