...
const RegisterValidation: any = yup.object().shape({
    ...
    // .matches(/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[`~`!@#$%^&.,<>?/*:";'{}()=\[\]+_\-])[A-Za-z\d~`!@#$%^&.,<>?/*:";'{}()\[\]=+_\-]{8,15}$/, ValidationMessage?.PASSWORD_MATCH),
    ...
    agree: yup.boolean().required(ValidationMessage?.ANY_REQUIRED + "check.")
        .oneOf([true], "Please accept Privacy Policy and Terms & Conditions."),
